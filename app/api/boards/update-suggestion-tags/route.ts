...
export async function POST(request: Request) {
  ...
  const { tags, suggestionId, boardName } = body

  if (!tags || !Array.isArray(tags) || !suggestionId || !boardName) {
  ...
  try {
    ...
    if (board) {
      ...
      if (suggestionIndex !== -1) {
        board.suggestions[suggestionIndex].tags = tags

        await collection.updateOne({ urlName: boardName }, { $set: { suggestions: board.suggestions } })

        return NextResponse.json(
          {
            message: "Suggestion tags updated successfully",
          },
          { status: 200 },
        )
      } else {
